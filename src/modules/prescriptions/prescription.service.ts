...
export default class PrescriptionService {
  static async createPrescription(doctorUserId: string, input: CreatePrescriptionInput) {
    ...
    if (consultation.doctorId !== doctorId) throw new Error("Forbidden: Not your consultation");
  ...
  static async getPrescriptionById(id: string, userId: string, role: string) {
    ...
    if (role === "PATIENT" && p.consultation.patientId !== userId) throw new Error("Forbidden");
    if (role === "DOCTOR") {
      const doctorId = await getDoctorIdByUserId(userId);
      if (!doctorId || p.doctorId !== doctorId) throw new Error("Forbidden");
    }
  ...
  static async updatePrescription(id: string, doctorUserId: string, input: UpdatePrescriptionInput) {
    ...
    if (!p) throw new Error("Prescription not found");
    if (p.doctorId !== doctorId) throw new Error("Forbidden");
  ...
  static async downloadPrescription(id: string, userId: string, role: string) {
    ...
    // Instead of generating a PDF (extra dependency), return a base64-encoded text summary
    const lines: string[] = [];
